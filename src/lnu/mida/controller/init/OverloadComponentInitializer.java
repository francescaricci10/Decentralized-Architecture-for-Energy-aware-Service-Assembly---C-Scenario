	...
	public boolean execute() {
		...
		for (int i = 0; i < Network.size(); i++) {
			...
			for (int j = 0; j < services_per_node; j++) {
				...
				s.setTransfer_func_CPU(new CustomTransferFunction(14*Math.random()));
				...
				for (int k = 0; k < types; k++) {
					transfer_func[k] = new CustomTransferFunction(14*CommonState.r.nextDouble()); // transfer_func[j] = new
